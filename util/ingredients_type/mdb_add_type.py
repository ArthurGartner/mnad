...
def update_type_in_mongodb(json_file_path):
    ...
    for ingredient, ing_type in type_data.items():
        query = {"name": ingredient}
        new_values = {"$set": {"type": ing_type}}
        
        # Update the document in MongoDB
        collection.update_one(query, new_values)
        print(f"Updated {ingredient}'s type to {ing_type}")

# File path to the JSON file containing the name:abv pairs
json_file_path = "C:\\Users\\Arthur.Gartner\\OneDrive\\Documents\\Personal\\Projects\\mnad\\ingredients_types.json"

# Run the function to update MongoDB documents
update_type_in_mongodb(json_file_path)
